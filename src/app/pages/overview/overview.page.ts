...
export class OverviewPage implements OnInit {
  ...
  constructor(private modalCtrl: ModalController, private routerOutlet: IonRouterOutlet, private dataService: DataService, private popoverCtrl: PopoverController) { }
  ...
  async openProjectPopover(event) {
    ...
    popover.onDidDismiss()
      .then(res => {
        ...
        console.log('my popover result', res);

        if (res.data && res.data.project) {
          this.task.project = res.data.project;
        }
      });
  }
  ...
  searchTask(event) {
    ...
    this.dataService.searchTask(term.toLowerCase())
      .then(results => {
